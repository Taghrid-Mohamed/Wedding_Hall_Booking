 ...
 private UserService_MA userService = new UserService_MA();
    ...
    try {
       ...
       String username = txtUsername.getText().trim();
        ...
        User_MA loggedUser = userService.login(username, password);   // نحاول نسجّل دخول

    if (loggedUser != null) {                                     // لو رجع لنا يوزر (دخول صحيح)
    JOptionPane.showMessageDialog(this,
            "Login Successful! Welcome " + loggedUser.getUsername(),
            "Success",
            JOptionPane.INFORMATION_MESSAGE);

    String role = loggedUser.getRole();                        // نجيب الدور من اليوزر (admin / employee)

    if ("admin".equalsIgnoreCase(role)) {                      // لو الدور أدمن
        AdminWindow_TM adminWin = new AdminWindow_TM();        // نفتح واجهة الأدمن
        adminWin.setLocationRelativeTo(null);
        adminWin.setVisible(true);
    } else {                                                   // أي دور ثاني نعتبره موظف
        MainWindow_TM empWin = new MainWindow_TM();            // نفتح واجهة الموظف
        empWin.setLocationRelativeTo(null);
        empWin.setVisible(true);
    }

    this.dispose();                                            // نقفل شاشة اللوقين
} else {
    JOptionPane.showMessageDialog(this,
